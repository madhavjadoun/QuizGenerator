      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-1.5 mt-4 pt-3 border-t border-[var(--border)] min-w-0">
        <div className="flex gap-1.5 min-w-0 flex-1">
          {attempt.completed ? (
            ...
            // ponytail: quiz is generated but not yet attempted — link to existing quiz, not docId (which costs another credit)
            <button
              onClick={() => router.push(`/chat?quizId=${quiz.id}`)}
              className="flex-1 min-w-0 py-1.5 text-center text-[11px] font-bold rounded-xl transition-all cursor-pointer h-8 flex items-center justify-center gap-1 border border-[var(--border)] hover:bg-[var(--bg-2)] text-[var(--text-2)]"
            >
              Play Quiz
            </button>
          )}
